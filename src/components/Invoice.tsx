      ...
      <div ref={invoiceRef} className="w-full max-w-3xl bg-card shadow-2xl rounded-2xl overflow-hidden print-container">
        ...
        <div className="invoice-gradient px-4 py-5 md:px-10 md:py-6 space-y-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex flex-col md:flex-row md:flex-wrap gap-2 md:gap-x-6 md:gap-y-2 text-sm text-primary-foreground/90 w-full md:w-auto md:justify-end">
              {invoiceData.from.email && (
                <a
                  href={`mailto:${invoiceData.from.email}`}
                  className="flex items-center gap-2 hover:text-primary-foreground transition-colors"
                >
                  <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  <span className="truncate underline-offset-2 hover:underline">{invoiceData.from.email}</span>
                </a>
              )}
              {invoiceData.from.phone && (
                <a
                  href={`tel:${invoiceData.from.phone.replace(/\s+/g, '')}`}
                  className="flex items-center gap-2 hover:text-primary-foreground transition-colors"
                >
                  <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                  <span className="underline-offset-2 hover:underline">{invoiceData.from.phone}</span>
                </a>
              )}
              {invoiceData.from.address && (
                <div className="flex items-start gap-2">
                  ...
                  <span className="whitespace-pre-line">{invoiceData.from.address}</span>
                </div>
              )}
            </div>
          </div>
